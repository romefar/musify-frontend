...
export const SearchArtist = () => {
  ...
  return (
    <section className={styles.searchArtistContainer}>
      ...
      {data && data.searchArtist.artists.length && (
        <Typography.Paragraph className={styles.resultCount}>{`Found ${data.searchArtist.totalResults} artists`}</Typography.Paragraph>
      )}
      {searchText && !data?.searchArtist.artists.length && !loading && (
